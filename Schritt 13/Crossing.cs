namespace Ampel
{
   ...
   public partial class Crossing : Form
   {
      ...
      public void AddDurationSum()
      {
         trackBar.Maximum = 0;
         foreach (var item in MainPhaseQueue)
         {
            trackBar.Maximum += item.Duration;
         }
      }
      private void CarHasArrived(object sender, EventArgs e)
      {
         if (MainPhaseQueue.Count == 0)
         {
      ...
      {
         label.Text = trackBar.Value.ToString("00");
      }

      private void trackBar_ValueChanged(object sender, EventArgs e)
      {
         MainController.GoToTime(trackBar.Value);
      }
   }
}